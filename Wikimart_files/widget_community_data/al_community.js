...
changeGroupState: function(state, callback) {
  ...
  if (cur.noAuth) {
    ...
    window.gotSession = function(autorzied) {
      ...
      if (autorzied) {
        addClass(cur.join_community, 'color1_bg');
        addClass(cur.join_community, 'community_checked');
        removeClass(cur.join_community, 'color1_bg');
        removeClass(cur.join_community, 'color1');
        cur.noAuth = false;
        cur.justAuth = true;
        ajax.post('/widget_community.php', {act: 'a_get_info', oid: cur.oid}, {
          onDone: function(result) {
            if (result.hash) {
              ...
              Community.sendChangeState(state, cur.oid, callback);
            }
          }
        });
      } else {
